...
DEFAULT_SUBFOLDER = "Year/Year-Month/Year-Month-Day"

LIGHT = {
    "bg": "#EEF0F0", "surface": "#FFFFFF", "ink": "#171A19",
    "muted": "#6B716F", "border": "#D7DBDA", "accent": "#2F6F4E",
    "select_fg": "#FFFFFF",
}
DARK = {
    "bg": "#1B1E1D", "surface": "#242827", "ink": "#EDEFEE",
    "muted": "#9AA19E", "border": "#3A3F3D", "accent": "#4F9E76",
    "select_fg": "#0E1110",
}
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.dest_dir = None
        self._muted_labels = []  # secondary-text labels that need re-tinting on theme change
    ...
    def _resource_path(self, filename):
        """Resolve a bundled asset both when run from source and when
        packaged by PyInstaller (which unpacks assets to sys._MEIPASS)."""
        base = getattr(sys, "_MEIPASS", os.path.dirname(os.path.abspath(__file__)))
        return os.path.join(base, filename)

    def _set_window_icon(self):
        try:
            icon_img = tk.PhotoImage(file=self._resource_path("ssd_icon.png"))
            self.iconphoto(True, icon_img)
            self._icon_img = icon_img  # keep a reference so it isn't garbage-collected
        except Exception:
            pass  # icon is cosmetic — never block the app over a missing/unsupported file
    ...
    def _apply_theme(self):
        ...
        s.configure("TLabelframe", background=p["bg"], foreground=p["ink"], bordercolor=p["border"])
        s.configure("TLabelframe.Label", background=p["bg"], foreground=p["ink"])
        s.configure("TLabel", background=p["bg"], foreground=p["ink"])
        s.configure("TCheckbutton", background=p["bg"], foreground=p["ink"])
        s.map("TCheckbutton", background=[("active", p["bg"])])
        s.configure(
            "TButton", background=p["surface"], foreground=p["ink"],
            bordercolor=p["border"], focusthickness=1,
        )
        s.map("TButton", background=[("active", p["border"])])
        s.configure(
            "TEntry", fieldbackground=p["surface"], foreground=p["ink"],
            bordercolor=p["border"], insertcolor=p["ink"],
        )
        s.configure(
            "Treeview", background=p["surface"], fieldbackground=p["surface"],
            foreground=p["ink"], bordercolor=p["border"],
        )
        s.configure("Treeview.Heading", background=p["border"], foreground=p["ink"])
        s.map("Treeview", background=[("selected", p["accent"])], foreground=[("selected", p["select_fg"])])

        for lbl in self._muted_labels:
            lbl.configure(foreground=p["muted"])

        if hasattr(self, "status"):
            self.status.configure(background=p["bg"])
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(
            top_row, text="Dark mode", variable=self.dark_mode,
       ...
       # Step 4: destination + copy
        ...
        note1.pack(anchor="w", padx=10, pady=(0, 10))
        note2 = ttk.Label(
            f4,
            text="Folders are created if they don't exist yet, and reused if they do.",
        )
        note2.pack(anchor="w", padx=10, pady=(0, 0))
        self._muted_labels.extend([note1, note2])
